...
from app.models import team_model, user_model,task_model,enterprise_model
...
async def add_new_task(task_data:CreateTask,db:db_dependency,
    ...
    if not team:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Team not found or not in your enterprise")
    ...
    try:
        if current_user.role == 'admin':
            tasks = db.query(task_model.Task).join(team_model.Team,
                                                   ...
                                                   ).filter(team_model.Team.enterprise_id == current_user.enterprise_id).all()
            return tasks
        else:
            tasks = db.query(task_model.Task).join(team_model.Team,
                                                   team_model.Team.id == task_model.Task.team_id
