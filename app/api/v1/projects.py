...
from app.models import team_model, enterprise_model, user_model, Enterprise
...
async def show_projects(db:db_dependency,current_user: user_model.User = Depends(get_current_user)):
    ...
    projects = db.query(Project).filter(Project.enterprise_id == current_user.enterprise_id).all()
    if not projects:
        raise HTTPException(
...
async def delete_project(project_id:int,db:db_dependency,current_user: user_model.User = Depends(get_current_user)):
    ...
    project = db.query(Project).filter(Project.id == project_id,
                                       Project.enterprise_id == current_user.enterprise_id).first()
...
@router.post('/assign-project/{project_id}/{team_id}',dependencies=[Depends(require_role(['manager','admin']))])
async def assign_project(project_id:int,team_id:int,db:db_dependency,current_user: user_model.User = Depends(get_current_user)):
    project = db.query(Project).filter(Project.id == project_id,Project.enterprise_id == current_user.enterprise_id).first()
    team = db.query(team_model.Team).filter(team_model.Team.id == team_id,team_model.Team.enterprise_id == current_user.enterprise_id).first()
    ...
    if project.enterprise_id != team.enterprise_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail='project and team do not belong to the same enterprise')
    try:
